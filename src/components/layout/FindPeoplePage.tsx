		...
		<section className="motion-enter flex min-w-0 flex-1 overflow-y-auto bg-[var(--bg-surface-alt)] p-3 sm:p-4 lg:p-6">
			<div className="mx-auto w-full max-w-5xl rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-5 shadow-sm sm:p-6">
				<div className="mb-5 border-b border-[var(--border)] pb-4">
					...
					<h2 className="mt-1 text-xl font-semibold tracking-tight text-[var(--text-primary)]">Find People</h2>
					<p className="mt-1 text-sm text-[var(--text-secondary)]">Search users by username and send friend requests.</p>
				</div>
				...
				<div className="mt-4 space-y-2">
					{filteredUsers.map((user) => {
						...
						return (
							<div key={user.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-soft)] p-3">
								<div className="flex min-w-0 items-center gap-3">
									<div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[var(--avatar-neutral-bg)] text-xs font-semibold text-white">
								...
								{friendshipState === 'friend' ? (
									<span className="rounded-full bg-[var(--accent-soft)] px-3 py-1 text-xs font-semibold text-[var(--accent)]">Friends</span>
								) : friendshipState === 'sent' ? (
									<span className="rounded-full border border-[var(--border)] px-3 py-1 text-xs font-medium text-[var(--text-secondary)]">Request Sent</span>
								) : friendshipState === 'incoming' ? (
									<span className="rounded-full border border-[var(--border)] px-3 py-1 text-xs font-medium text-[var(--text-secondary)]">Incoming Request</span>
								) : (
									<button
										type="button"
										onClick={() => void sendRequest(user.id)}
										disabled={isSending}
										className="motion-interactive inline-flex items-center gap-1 rounded-xl bg-[var(--accent)] px-3 py-2 text-xs font-semibold text-[var(--bg-page)] shadow-[var(--shadow-accent)] hover:bg-[var(--accent-strong)] disabled:cursor-not-allowed disabled:opacity-70"
									>
										<UserPlus size={14} />
